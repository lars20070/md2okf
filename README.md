...
Drop some Markdown files into `md/`, run `make wiki`, and the Pi
...
`pi-kit` here is the kit's name, which comes from `pi/spec.yaml`.
...
The driver names the path to `SKILL.md` in the prompt rather than calling
`/skill:compile-wiki`. `pi -p` passes a prompt through as typed and never
expands a slash command.

The kit is `pi/`, and the config it carries — `AGENTS.md`, `settings.json`,
`models.json` and `skills/` — lives in `pi/files/home/.pi/agent/`. It is copied
into the sandbox at kit build time, not mounted, so an edit reaches Pi on the
next fresh sandbox, which `make wiki` always builds.

### Model configuration

The `openrouter` entry in `pi/files/home/.pi/agent/models.json` names the
...
1. **Name the gateway.** In `pi/files/home/.pi/agent/models.json`, put your
   gateway's URL in `baseUrl` in place of the `litellm.example.com` placeholder,
   and the model you want in `models`.
2. **Choose it.** In `pi/files/home/.pi/agent/settings.json`, set
...
3. **Open the road.** Add the gateway's host to `caps.network.allow` in
   `pi/spec.yaml`, and give it a `credentials` entry like the OpenRouter one:
...
- **On the host**, `make lint-okf` runs it through `pnpm dlx`, so there is
  ...
  outside `make lint` and outside CI, and why the driver does not call it.
- **In the sandbox**, the kit installs okf-lint at a pinned `OKF_LINT_VERSION`
  (`pi/spec.yaml`) and the `compile-wiki` skill wraps it in
