...
# Drop into interactive Pi at the workspace (okf/, the wiki root). `sbx exec`
# starts the sandbox first if it is stopped. `-it` keeps a real TTY — do not
# redirect stdin (that is only for non-interactive `pi -p`, as in compile-okf.sh).
