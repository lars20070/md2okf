...
You are Codex running inside a Docker Sandbox microVM, driven by md2okf. Your
workspace is the OKF wiki itself, mounted read-write, and edits apply directly
to the host working tree. Four more host paths are mounted:
...
- `$MD2OKF_STATE_DIR/sessions` — writable persistent state for your transcripts
...
configuration. Codex writes its transcripts to its native `~/.codex/sessions`
path, which is bind-mounted onto `$MD2OKF_STATE_DIR/sessions`. A write
...
This file is your global `AGENTS.md` (`~/.codex/AGENTS.md`). The wiki is not a
git repository, so no project `AGENTS.md` applies; `../SPEC.md` outranks this
file.

`sudo` is passwordless. If a host is blocked by policy, stop and ask the user to
allow it with `sbx policy allow network "<host>"`; do not work around the block.
...
The procedure for each task lives in a skill under `~/.agents/skills/`, one
directory per task, each with a `SKILL.md`.

- When a run names a skill (for example `$compile-okf`), **read its
  `SKILL.md` first** and follow it. The
...
tags:
  - example-tag
  - another-tag
generated: { by: codex/<model-id>, at: "2026-09-18T06:28:39Z" }
---
```
...
- `generated` — who wrote the page and when (spec §5.2). `by` is an actor in the
  §7 form `<producer>/<version>`: you are Codex, so it is `codex/` followed by
  the identifier of the model you are running, lower-case, e.g. `codex/gpt-5`.
  `at` is an ISO 8601 datetime with an
