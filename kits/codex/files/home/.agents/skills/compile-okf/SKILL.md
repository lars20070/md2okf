...
4. **Write the content pages**, organised into directories by topic, each with
   ...
   then read each section's line range (`sed -n 'START,ENDp'`) — never pull a
   whole book into one call.
...
A patch or command is part of your reply, so it is subject to the same output
limit as your prose. One that carries a whole long page can be cut off
mid-argument, and then **all of that content is lost** — nothing reaches disk.

- Build a long page **incrementally**: create it with the frontmatter plus the
  first section, then add each next section after the last one in a further
  edit, and so on. Each edit should carry a section or two, not a whole chapter.
- Read long sources the same way — a line range at a time (`sed -n
  'START,ENDp'` over the ranges `inspectmd` reports), rather than pulling an
  entire book into one command.
- If an edit does get cut off or rejected, do not retry it unchanged. Split the
...
- **Before finishing every run**, after the pages are written and the indexes
  ...
  ```bash
  ~/.agents/skills/compile-okf/scripts/check-okf.sh
  ```
